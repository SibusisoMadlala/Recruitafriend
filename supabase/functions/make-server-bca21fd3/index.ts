...
app.get('/make-server-bca21fd3/subscriptions/current', async (c) => {
  try {
    const user = await getAuthUser(c.req.header('Authorization'), c.req.header('x-rf-user-jwt'));
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    ...
    if (error) throw error;
    if (!data) return c.json({ subscription: 'starter', plan: null, status: 'none' });

    return c.json({ subscription: data.plan_id, status: data.status, data });
  } catch (error) {
