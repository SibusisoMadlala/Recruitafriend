...
app.get('/make-server-bca21fd3/employer/talent-search', async (c) => {
  try {
    ...
    let query = db
      .from('profiles')
      .select('id, name, email, headline, summary, location, avatar_url, skills, social_links, updated_at, cv_years_experience, cv_ai_summary, cv_job_titles, cv_qualifications, has_cv')
      .eq('user_type', 'seeker')
      .limit(500);
...
app.get('/make-server-bca21fd3/profile/views', async (c) => {
  ...
  if (!user) return c.json({ error: 'Unauthorized' }, 401);
  const db = getDb();
  // Count applications where employer moved past 'applied' = they viewed the profile
