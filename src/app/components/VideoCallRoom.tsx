  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      ...
      try {
        ...
        const lossRate = total > 0 ? lost / (total + lost) : 0;
        setQuality(lossRate < 0.05 ? 'good' : 'poor');
      } catch {}
    }, 5000);
    return () => clearInterval(t);
  }, [status]);
  ...
  const toggleScreen = useCallback(async () => {
    ...
    if (screenOn) {
      ...
      try {
        ...
        screenTrack.onended = () => toggleScreen(); // auto-stop when user closes share
        setScreenOn(true);
      } catch {}
    }
  }, [screenOn]);
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#0d1117', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#0A2540', borderBottom: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {status === 'connected' && (
            quality === 'good'
              ? <Wifi size={14} color="#00C853" title="Good connection" />
              : quality === 'poor'
                ? <WifiOff size={14} color="#f59e0b" title="Poor connection" />
                : null
          )}
          {/* Status badge */}
